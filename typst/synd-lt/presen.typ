...
#west-slide(title: "RustでTUI")[
  #figure(
    image("./images/ss-1.png", width: 80%, height: 80%, fit: "contain"),
    numbering: none,
...
#west-slide(title: "RustでTUI")[
  #figure(
    image("./images/ss-2.png", width: 80%, height: 80%, fit: "contain"),
    numbering: none,
    caption: [ Feedの一覧 ],
  )
]
...
#west-slide(title: "RustのTUI library")[
  ...
  + ratatui(tui widgets library)
]

#west-slide(title: "RustのTUI library")[
  #figure(
    image("./images/ratatui-ss.png", width: 80%, height: 80%, fit: "contain"),
    numbering: none,
    caption: [ Netflixが公開したeBPFのtui toolでもratatuiが使われている ],
  )
]
#west-slide(title: "ratatui")[
  ...
  TableやList等の基本的なwidgetが用意されている
]
...
```
project rootのflake.nixで開発時の依存を宣言
]

#west-slide(title: "nix develop")[
  #figure(
    image("./images/nix-develop-1.png", width: 100%, height: 40%, fit: "stretch"),
    numbering: none,
    caption: [ nix developを実行すると宣言したpackageがPATHに入った状態で開発環境(shell)が立ち上がる ],
  )
]
...
#west-slide(title: "Nixで管理すると何がうれしいのか")[
  ...
  #text(fill: gray.lighten(50%),[- 開発環境をNixで管理できる])
  - CIをNixで管理できる
]
...
#west-slide(title: "Nixで管理すると何がうれしいのか")[
  #text(fill: gray.lighten(50%),[- 開発環境をNixで管理できる])
  #text(fill: gray.lighten(50%),[- CIをNixで管理できる])
  - CacheをNixで管理できる 
]
...
#west-slide(title: "Nixで管理すると何がうれしいのか")[
  #figure(
    image("./images/ci-cache.png", width: 100%, height: 40%, fit: "contain"),
    numbering: none,
    caption: [ cacheから取得 ],
  )
]
...
#west-slide(title: "Nixで管理すると何がうれしいのか")[
  #text(fill: gray.lighten(50%),[- 開発環境をNixで管理できる])
  #text(fill: gray.lighten(50%),[- CIをNixで管理できる])
  #text(fill: gray.lighten(50%),[- CacheをNixで管理できる])
  - InstallをNixで管理できる
]
...
#west-slide(title: "Nixで管理すると何がうれしいのか")[
  #figure(  
    image("./images/run-ss.png", width: 100%, height: 40%, fit: "contain"),
    numbering: none,
    caption: [ nix runで一時的に実行したり \ nix profileでinstallもできる \
    MacとLinuxで同じpackage管理ができる ],
  )
]
...
#west-slide(title: "Nixで管理すると何がうれしいのか")[
  ...
  #text(fill: gray.lighten(50%),[- CIをNixで管理できる])
  #text(fill: gray.lighten(50%),[- CacheをNixで管理できる])
  #text(fill: gray.lighten(50%),[- InstallをNixで管理できる])
  - DeployをNixで管理できる
]
...
#west-slide(title: "NixでDeploy")[
  #figure(  
    image("./images/deploy-rs.png", width: 100%, height: 80%, fit: "contain"),
    numbering: none,
    caption: [ deploy-rsでNixOSが入ったRaspberryPiにdeploy ]
  )
]
...
#west-slide(title: "cargo-distがやってくれる")[
  #figure(  
    image("./images/release-ss.png", width: 100%, height: 80%, fit: "contain"),
    numbering: none,
...
#west-slide(title: "cargo-distがやってくれる")[
  ...
  - 各種platform(aarch64,x86のdarwin,windows, linux gnu,musl)向けのbinary生成
  - shell,powershellのinstall script作成
  - homebrew用repositoryの更新(push権限を渡す必要あり)
]
#west-slide(title: "cargo-distがやってくれる")[
  #figure(  
    image("./images/cargo-dist-plan.png", width: 90%, height: 60%, fit: "contain"),
    numbering: none,
    caption: [ `cargo dist plan`を実行するとCIで実行される内容を確認できる \ workflow yamlにベタ書きされているのではなく、`cargo dist plan`の出力を参照するようになっている ]
  )
]
...
#west-slide(title: "RustでOpenTelemetry")[
    #figure(  
    image("./images/dashboard-ss.png", width: 100%, height: 80%, fit: "contain"),
    numbering: none,
...
#west-slide(title: "RustでOpenTelemetry")[
    #figure(  
    image("./images/trace-ss.png", width: 100%, height: 80%, fit: "contain"),
    numbering: none,
    caption: [ Entry取得時のTrace ]
  )
]
