...
description = "kernelのsamples/rust/rust_driver_*.rsを読むための本"
date = "2026-02-21"
draft = true
[taxonomies]
tags = ["linux","book"]
...
Rustのユースケースとして、[driverをrustで書く取り組み](https://github.com/Rust-for-Linux/linux/blob/b8d687c7eeb52d0353ac27c4f71594a2e6aa365f/samples/rust/rust_driver_pci.rs)があることを知り、kernel driverの解説を探している中で本書を見つけ、読んでみておもしろかったので、感想をかきます。
...
`insmod`時に呼ばれる、`module_init`, `rmmod`時の`module_exit`だけでなく、systemcall(open,close,ioctol,epoll,...)時のhandler, 割込み、timerからデバイスの処理が呼ばれます。。
...
epollの詳細は割愛しますが、これを実行すると無事redableであることを伝えられました

```sh
2026-02-08T09:24:22.709203Z  INFO devone_epoll: epoll_wait retruned n=1
2026-02-08T09:24:22.709280Z  INFO devone_epoll: event[0]: token=123 events=0x1
2026-02-08T09:24:22.709301Z  INFO devone_epoll:   => EPOLLIN (redable)
```
...
カーネルのメモリ管理について。具体的には、`kmalloc()`,`kfree()`といったAPI、スラブアロケータ、パディシステム、カーネルスタックについての言及があります。
...
`ioaddr`は事前にstackに確保してある前提のようです。
...
I2C(SMBus)について。
プロトコルの概要や、バス制御方法の説明があります。自分は組み込みに馴染がないので、このあたりの解説は非常に参考になりました。
また、lm_sensorsパッケージの説明もあります。
実装については、kernelのI2Cが`i2c-dev`, `i2c-core`, `i2c-alog`, i2c adapterといったモジュールスタックからなるといった解説があります。
